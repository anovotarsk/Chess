...
ChessLogic::ChessLogic() {
    ...
    m_check = false;
    m_checkmate = No;
    last_move = std::pair<int, int>(0, 0);
...
ChessColor ChessLogic::getCheckmate() {
    return m_checkmate;
}
...
void ChessLogic::moveFigure(int from, int to) {
    ...
    if (m_board[to].second == Pawn) {
        if (m_board[to].first == White
            && to + 8 == last_move.second
            && to - 8 == last_move.first) {
            m_board[last_move.second] = std::pair<ChessColor, ChessFigure>(No, None);
        }
        if (m_board[to].first == Black
            && to - 8 == last_move.second
            && to + 8 == last_move.first) {
            m_board[last_move.second] = std::pair<ChessColor, ChessFigure>(No, None);
        }
    }
    last_move = std::pair<int, int>(from, to);
    ...
    m_checkmate = Stalemate;
    for (int i = 0; i < 64; i++) {
        if (m_board[i].first == m_turn) {
    ...
    if (m_checkmate == Stalemate && m_check == true)
        m_checkmate = m_turn;
    //}

    if (m_board[to].second == Pawn) {
        ChessFigure figure = Pawn;

        if ((m_board[to].first == White && getY(to) == 0)
            || (m_board[to].first == Black && getY(to) == 7)) {
            QMessageBox msgBox;
            QPushButton *rook = msgBox.addButton(QObject::tr("Rook"), QMessageBox::ActionRole);
            QPushButton *knight = msgBox.addButton(QObject::tr("Knight"), QMessageBox::ActionRole);
            QPushButton *bishop = msgBox.addButton(QObject::tr("Bishop"), QMessageBox::ActionRole);
            QPushButton *queen = msgBox.addButton(QObject::tr("Queen"), QMessageBox::ActionRole);
            ...
            if (msgBox.clickedButton() == rook) {
                ...
                if(msgBox.clickedButton() == knight) {
                    figure = Knight;
                }
                else {
        ...
        m_board[to] = std::pair<ChessColor, ChessFigure>(m_board[to].first, figure);
    }
}
...
std::list<int> ChessLogic::pawnWays(int area, bool check) {
    ...
    if (m_board[area].first == ChessColor::White) {
        if (area - 8 > -1 && m_board[area - 8].first == ChessColor::No) {
        ...
        if (getX(area) < 7 && getY(area) > 0 && m_board[area - 7].first == ChessColor::Black)
            ways.push_back(area - 7);
        if (getY(last_move.first) == 1
            && getY(last_move.second) == 3
            && m_board[last_move.second].second == Pawn
            && getY(last_move.second) == getY(area)) {
            if (getX(area) > 0 && area - 1 == last_move.second)
                ways.push_back(area - 9);
            else {
                if (getX(area) < 7 && area + 1 == last_move.second)
                    ways.push_back(area - 7);
            }
        }
    }
    if (m_board[area].first == ChessColor::Black) {
        ...
        if (getX(area) < 7 && getY(area) < 7 && m_board[area + 9].first == ChessColor::White)
            ways.push_back(area + 9);
        if (getY(last_move.first) == 6
            && getY(last_move.second) == 4
            && m_board[last_move.second].second == Pawn
            && getY(last_move.second) == getY(area)) {
            if (getX(area) > 0 && area - 1 == last_move.second)
                ways.push_back(area + 7);
    ...
    if (m_turn == m_board[area].first) {
        auto save_board = m_board;
        std::list<int> checked_ways;

        for (int i : ways) {
            m_board[i] = m_board[area];
            m_board[area] = std::pair<ChessColor, ChessFigure>(No, None);
            if (enemy_atack_king() == false) {
                checked_ways.push_back(i);
            }
            m_board = save_board;
        }
        ways = checked_ways;
